    ...
    return new Promise((resolve, reject) => {
        ...
        get_firebase_certs(now)
            .then((certs) => {
                ...
                if (!key)
                    return response('Inavalid token', null, cb, resolve, reject);

                let verified;
                try {
                    verified = jwt.decode(token, key);
                } catch (err) {
                    return response('Invalid token', cb, resolve, reject);
                }
...
const validate = (token, projectId, p1, p2) => {
    return new Promise((resolve, reject) => {
        ...
        if (!projectId)
            return response('Invalid token', null, cb, resolve, reject);
